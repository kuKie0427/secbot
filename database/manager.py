...
class DatabaseManager:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        if db_path:
            ...
            if db_url and db_url.startswith("sqlite:///"):
                ...
                # 默认路径
                self.db_path = Path(settings.project_root) / "data" / "m_bot.db"
    ...
    def _init_database(self):
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_crawler_tasks_status ON crawler_tasks(status)")
    ...
    def save_conversation(self, conversation: Conversation) -> int:
        ...
        with self._get_connection() as conn:
            ...
            """, (
                ...
                json.dumps(conversation.metadata or {}) if conversation.metadata else None
            ))
    ...
    def list_prompt_chains(self) -> List[PromptChainModel]:
        ...
        with self._get_connection() as conn:
            ...
            for row in rows:
                metadata = json.loads(row["metadata"]) if row["metadata"] else {}
                chains.append(PromptChainModel(
                    id=row["id"],
                    name=row["name"],
                    content=row["content"],
                    description=row["description"],
    ...
    def list_configs(self, category: Optional[str] = None) -> List[UserConfig]:
        ...
        with self._get_connection() as conn:
            ...
            if category:
                cursor.execute("SELECT * FROM user_configs WHERE category = ? ORDER BY key", (category,))
            else:
    ...
    def update_crawler_task(self, task_id: int, status: Optional[str] = None, result: Optional[Any] = None):
        """更新爬虫任务"""
        with self._get_connection() as conn:
            cursor = conn.cursor()
