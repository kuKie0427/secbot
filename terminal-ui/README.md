...
基于 Ink (React for CLI) 的终端 TUI，通过 HTTP/SSE 连接 Python 后端，核心逻辑仍在 Python 侧。
...
- **或**：先打开系统自带的 **CMD / PowerShell / Windows Terminal**，`cd` 到项目根，再执行 `uv run python main.py`（同一窗口会先起后端再进 TUI）。
...
- 本 TUI 为独立 Node 进程，通过 HTTP/SSE 与后端通信
- Python 自带的交互模式（`python main.py` 无参数）仍保留，为「传统」入口；本 TUI 为「新方式」，需先启动 router 再启动本包
