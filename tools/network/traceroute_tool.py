"""路由追踪工具：追踪到目标的网络路径"""
import asyncio
import platform
from typing import Any, Dict
from tools.base import BaseTool, ToolResult


class TracerouteTool(BaseTool):
    """路由追踪工具：追踪数据包到达目标经过的路由节点"""

    sensitivity = "low"

    def __init__(self):
        super().__init__(
            name="traceroute",
            description="追踪到目标的网络路由路径（经过的每一跳路由器）。参数: target(目标IP/域名), max_hops(最大跳数, 默认30)",
        )

    async def execute(self, **kwargs) -> ToolResult:
        target = kwargs.get("target", "")
        if not target:
            return ToolResult(success=False, result=None, error="缺少参数: target")

        max_hops = int(kwargs.get("max_hops", 30))

        try:
            system = platform.system().lower()
            if system == "windows":
                cmd = ["tracert", "-d", "-h", str(max_hops), target]
            else:
                cmd = ["traceroute", "-n", "-m", str(max_hops), target]

            proc = await asyncio.create_subprocess_exec(
                *cmd,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=60)
            output = stdout.decode(errors="ignore")

            # 解析跳数
            hops = []
            for line in output.strip().split("\n"):
                line = line.strip()
                if not line or line.startswith("traceroute") or line.startswith("Tracing"):
                    continue
                parts = line.split()
                if parts and parts[0].isdigit():
                    hop_num = int(parts[0])
                    hop_ips = []
                    hop_times = []
                    for p in parts[1:]:
                        if p == "*":
                            hop_times.append("*")
                        elif p.replace(".", "").isdigit() and "." in p:
                            hop_ips.append(p)
                        elif p.endswith("ms"):
                            hop_times.append(p)
                    hops.append({
                        "hop": hop_num,
                        "ip": hop_ips[0] if hop_ips else "*",
                        "times": hop_times,
                    })

            return ToolResult(
                success=True,
                result={
                    "target": target,
                    "total_hops": len(hops),
                    "hops": hops,
                    "raw_output": output[:2000],
                },
            )
        except asyncio.TimeoutError:
            return ToolResult(success=False, result=None, error="追踪超时")
        except Exception as e:
            return ToolResult(success=False, result=None, error=str(e))

    def get_schema(self) -> Dict[str, Any]:
        return {
            "name": self.name,
            "description": self.description,
            "sensitivity": self.sensitivity,
            "parameters": {
                "target": {"type": "string", "description": "目标 IP 或域名", "required": True},
                "max_hops": {"type": "integer", "description": "最大跳数", "default": 30},
            },
        }
