...
- 使用 Ollama 时：本机已启动 `ollama serve`，且已 pull 所需模型（如 `gpt-oss:20b`）。
...
**目的**：先制定多步计划，再一次性执行（先扫描再漏洞扫描再报告）。

1. 输入：`/plan`
2. 在计划模式下输入：**对 192.168.56.10 先做端口扫描，再对开放的服务做漏洞扫描，最后生成简要报告**
3. 确认计划生成后输入：`/start`

**预期行为**：Planner 生成多步任务，执行时依次调用 port_scan → vuln_scan → 报告生成相关工具。
...
或使用计划模式一条龙：先 `/plan`，输入「对 192.168.56.10 先做端口扫描，再对开放服务做漏洞扫描，最后生成简要报告」，再 `/start`。
...
| 计划模式不执行 | 是否先输入 `/plan` 并看到「计划已生成」再输入 `/start` |
