...
class ToolCallingAgent(BaseAgent):
    """带工具调用的对话智能体，基于 LangChain 进行任务编排与工具调用"""
    ...
    def __init__(
        ...
        # 初始化工具描述生成器
        self.tool_desc_generator = (
            ToolDescriptionGenerator(self.tools) if self.tools else None
        )
        ...
        if langchain_tools:
            ...
            if self.tool_desc_generator:
                ...
                logger.debug(
                    f"系统提示词已增强，包含 {self.tool_desc_generator.get_tools_count()} 个工具的描述"
                )
        else:
            self.llm_with_tools = self.llm
            self.tools_dict = {}
    ...
    def get_tools_info(self) -> Dict[str, Any]:
        ...
        if not self.tool_desc_generator:
            return {"count": 0, "names": [], "summary": "无可用工具"}
        ...
        return {
            ...
            "description": self.tool_desc_generator.get_tools_description(
                format="text"
            ),
        }
            ...
            if self.tools_dict:
                ...
                if tool_calls:
                    ...
                    for tool_call in tool_calls:
                        ...
                        # 记录调试信息
                        logger.debug(
                            f"解析工具调用: name={tool_name}, args={tool_args}, 原始调用={tool_call}"
                        )
                        ...
                        if tool_name and tool_name in self.tools_dict:
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    if (
                                        isinstance(tool_args.get("args"), list)
                                        and len(tool_args["args"]) == 1
                                    ):
