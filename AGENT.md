...
`Client -> /api/chat -> SessionManager -> IntentRouter（6 类意图）-> [可选 ExploreAgent] -> ContextStore + ContextAssembler（预算装配）-> task_simple: ReAct 直跑 | task_complex: Planner -> TaskExecutor -> ... ->（needs_report 时）Summary -> SSE`

附带事件（与 npm terminal-ui 对齐）：`intent_decision`、`explore_*`、`context_patch`、`context_usage`、`clarify` 等。

环境变量：`SECBOT_EXPLORE_MAX_ITERS`、`SECBOT_CONTEXT_DEBUG`、`SECBOT_ADAPTIVE_REPLAN`（规划穿插）。
---
...
- `router/chat.py`
  - `/api/chat` SSE 入口
  - 事件到 SSE 的映射函数 `_event_to_sse`（含 `intent_decision`、`explore_*`、`context_patch`、`context_usage`）
  - root 权限请求/回传
- `secbot_agent/core/session.py`
  - 会话编排核心：`IntentRouter` 分类与会话类早退 → 可选 `ExploreAgent` → 带模型窗口预算的上下文装配 → `task_simple` 跳过 Planner 直跑 ReAct / `task_complex` 走 Planner+Executor → 仅当 `needs_report` 且非 `task_simple` 时 `SummaryAgent`
- `router/dependencies.py`
  - 进程级单例：`ContextStore`、`ContextAssembler`（共享同一 store）、`IntentRouter`、`ExploreAgent` 工具集
- `secbot_agent/core/context_store.py` / `secbot_agent/core/context_assembler.py` / `secbot_agent/core/model_context_window.py`
  - 上下文池与 token 预算切片（对齐 npm）
- `secbot_agent/core/agents/intent_router.py`、`secbot_agent/core/agents/explore_agent.py`
  - 意图分类与只读探索补上下文

### 5.3 Agent 体系

- `secbot_agent/core/agents/router.py`
  - 历史三层分类（`qa` / `technical` / `other`）；主聊天链路已改为 `IntentRouter` 六类意图，此类可能被其他入口复用
- `secbot_agent/core/agents/intent_router.py`
  - 六类意图（`small_talk` / `meta` / `qa` / `clarify_needed` / `task_simple` / `task_complex`）与 `needs_explore` / `needs_report` / `focus`
- `secbot_agent/core/agents/explore_agent.py`
  - 只读探索：CVE/页面等补上下文，产出 `ContextPatch`
...
`secbot_agent/core/session.py` 的职责是：

- 接住请求并完成 `IntentRouter` 分类与事件 `intent_decision`
- 会话类意图早退（small_talk/meta/qa/clarify_needed）
- 按需 `ExploreAgent` 与 `ContextStore` 补丁
- 调用 `ContextAssembler.build` 并发射 `context_usage`
- `task_simple`：核心 Agent ReAct 直跑（跳过 Planner）；`task_complex`：Planner → 执行器 / ReAct
- 仅在 `needs_report` 且非 `task_simple` 时摘要
- 把流程事件桥接到 `EventBus`
