...
def exploit(
    ...
    exploit_type: str = typer.Option(
        "web", "--type", "-t", help="漏洞利用类型 (web/network/post)"
    ),
    payload_type: str = typer.Option(
        "sql_injection", "--payload", "-p", help="Payload类型"
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="保存结果到文件"
    ),
    ...
    async def _exploit():
        try:
            ...
            console.print(
                f"[yellow]利用类型: {exploit_type}, Payload: {payload_type}[/yellow]\n"
            )

            engine = ExploitEngine()
            result = await engine.execute_exploit(
                exploit_type=exploit_type, target=target, payload={"type": payload_type}
            )
...
def attack_chain(
    ...
    async def _attack_chain():
        try:
            ...
            console.print(
                "[yellow]警告：此操作将执行完整的渗透测试流程，请确保已获得授权！[/yellow]\n"
            )
...
def generate_payload(
    payload_type: str = typer.Argument(
        ..., help="Payload类型 (sql_injection/xss/command_injection/path_traversal)"
    ),
